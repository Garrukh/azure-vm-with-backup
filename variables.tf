...
variable "keyvault_rg_name" {
  description = "Resource group name in which the keyvault for admin password is located"
  type        = string
}
...
variable "enable_automatic_updates" {
  description = "(Optional) Specifies if Automatic Updates are Enabled for the Windows Virtual Machine. Changing this forces a new resource to be created. Defaults to false."
  type        = bool
  default     = false
}
...
variable "os_type" {
  description = "Type of os that will by used for this vm. Can be either of: [Windows, Linux]"
  type        = string
}
...
variable "certificate_store" {
  description = "(Required) The certificate store on the Virtual Machine where the certificate should be added. Only requited when os_type is set to 'Windows'"
  type        = string
  default     = null
}
