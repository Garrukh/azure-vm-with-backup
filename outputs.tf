...
output "private_ip_address" {
  value       = var.os_type == "Linux" ? azurerm_linux_virtual_machine.vm[0].private_ip_address : azurerm_windows_virtual_machine.vm[0].private_ip_address
  description = "The Primary Private IP Address assigned to this Virtual Machine."
}

output "private_ip_addresses" {
  value       = var.os_type == "Linux" ? azurerm_linux_virtual_machine.vm[0].private_ip_addresses : azurerm_windows_virtual_machine.vm[0].private_ip_addresses
  description = "A list of Private IP Addresses assigned to this Virtual Machine."
}

output "public_ip_address" {
  value       = var.os_type == "Linux" ? azurerm_linux_virtual_machine.vm[0].public_ip_address : azurerm_windows_virtual_machine.vm[0].public_ip_address
  description = "The Primary Public IP Address assigned to this Virtual Machine."
}

output "public_ip_addresses" {
  value       = var.os_type == "Linux" ? azurerm_linux_virtual_machine.vm[0].public_ip_addresses : azurerm_windows_virtual_machine.vm[0].public_ip_addresses
  description = "A list of the Public IP Addresses assigned to this Virtual Machine."
}

output "virtual_machine_id" {
  value       = var.os_type == "Linux" ? azurerm_linux_virtual_machine.vm[0].virtual_machine_id : azurerm_windows_virtual_machine.vm[0].virtual_machine_id
  description = "A 128-bit identifier which uniquely identifies this Virtual Machine."
}

output "principal_id" {
  value       = var.identity_type != null ? ( var.os_type == "Linux" ? azurerm_linux_virtual_machine.vm[0].identity[0].principal_id : azurerm_windows_virtual_machine.vm[0].identity[0].principal_id ) : null
  description = "The ID of the System Managed Service Principal."
}

output "tenant_id" {
  value       = var.identity_type != null ? ( var.os_type == "Linux" ? azurerm_linux_virtual_machine.vm[0].identity[0].tenant_id : azurerm_windows_virtual_machine.vm[0].identity[0].tenant_id ) : null
